...
describe('computeCapabilities', () => {
  it('reports CSS Modules as supported when detected', () => {
    const caps = computeCapabilities(
      { hasCSSModules: true, hasTailwind: false, hasCSSInJS: false, hasPlainCSS: false, summary: '' },
  ...
  it('reports Tailwind as supported when resolver is available', () => {
    const caps = computeCapabilities(
      { hasCSSModules: false, hasTailwind: true, hasCSSInJS: false, hasPlainCSS: false, summary: '' },
  ...
  it('reports Tailwind as preview-only when detected but resolver unavailable', () => {
    const caps = computeCapabilities(
      { hasCSSModules: false, hasTailwind: true, tailwindVersion: 4, hasCSSInJS: false, hasPlainCSS: false, summary: '' },
      { resolverAvailable: false },
    )
    const tw = caps.find(c => c.name === 'Tailwind')
  ...
  it('reports CSS-in-JS as ai-required when detected', () => {
    const caps = computeCapabilities(
      { hasCSSModules: false, hasTailwind: false, hasCSSInJS: true, hasPlainCSS: false, summary: '' },
      { resolverAvailable: false },
    )
  ...
  it('returns empty array when only plain CSS detected', () => {
    const caps = computeCapabilities(
      { hasCSSModules: false, hasTailwind: false, hasCSSInJS: false, hasPlainCSS: true, summary: '' },
      { resolverAvailable: false },
    )
    expect(caps).toEqual([])
  })
